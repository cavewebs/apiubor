...
$matches = $conn->query("SELECT * FROM competition_seasons WHERE cs_md_status <41 ORDER BY cs_name ASC ") or die($conn->error);
while($row =  $matches->fetch_assoc()){
                echo 'the com being processed is: '.$row['cs_id'].'<br />';  

          try {
            $crawler = $client->request('GET', $row['cs_fixture_link']);
...
;
          ...
          if ($stmt){
              ...
              // $conns->commit();
              $update_fixtured = $conn->query("UPDATE competition_seasons set cs_md_status='41' WHERE cs_id='".$row['cs_id']."'");
              if($update_fixtured){
...
$conn = null;
echo "we Made it, We are for d2 rich";
var_dump($eroo)
?>;